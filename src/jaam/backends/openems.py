...
from jaam.ir import BoxOp, CurveOp, RotPolyOp, SimulationIR, WireOp
...
def _native_modules():
    try:
        ...
        raise NativeDependencyError(
            "openEMS backend requires CSXCAD/openEMS Python bindings; see docs/native-setup.md"
        ) from exc
...
def build_native(ir: SimulationIR):
    ...
    # smoothing_resolution_m (set by GradedMeshCoarseningPass) is the ceiling
    # actually intended for this mesh; max_resolution_m is the fine λ/20
    # validation threshold and would re-densify every coarsened region if
    # used here instead, silently undoing the pass.
    smoothing_res = ir.mesh.smoothing_resolution_m
    if smoothing_res is None:
        smoothing_res = ir.mesh.max_resolution_m
    for axis, lines in zip("xyz", (ir.mesh.lines_x, ir.mesh.lines_y, ir.mesh.lines_z)):
        grid.SetLines(axis, lines)
        grid.SmoothMeshLines(axis, smoothing_res, ratio=ir.mesh.grading_ratio)
    return csx, fdtd, ports


def run_openems(ir: SimulationIR, output_dir: Path, *, farfield: bool | str = True) -> RunResult:
    ...
    nf2ff = fdtd.CreateNF2FFBox() if quality != "off" else None
    ...
    if quality == "off" or nf2ff is None:
    ...
    n_theta, n_phi = (19, 37) if quality == "preview" else (181, 361)
    theta_deg = np.linspace(0.0, 180.0, n_theta)
    phi_deg = np.linspace(0.0, 360.0, n_phi)
